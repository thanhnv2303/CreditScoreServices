...
class CalculateX21:
    def __init__(self):
        ...
        self.wallets_age = [(time.time() - x['a.createdAt']) for x in self.data]
        self.mns, self.sstd = get_standardized_score_info(self.wallets_age)

        self.t_score = {self.data[i]['a.address']:((self.wallets_age[i] - self.mns)/self.sstd * 100 + 500) for i in range(len(self.wallets_age))}
...
class CalculateX22:
    def __init__(self, k = 100):
        ...
        self.sum_k_days = [sum(a['a.dailyTransactionAmounts'][0:k]) for a in self.data]
...
class CalculateX23:
    def __init__(self, k = 100):
        ...
        self.sum_k_days = [sum(a['a.dailyFrequencyOfTransactions'][0:k]) for a in self.data]
        self.mns, self.sstd = get_standardized_score_info(self.sum_k_days)
...
class CalculateX24:
    def __init__(self):
        ...
        self.numbers_of_liquidation = [a['a.numberOfLiquidation'] for a in self.data]
...
class CalculateX25:
    def __init__(self):
        ...
        self.total_amounts_liquidation = [a['a.totalAmountOfLiquidation'] for a in self.data]
        self.mns, self.sstd = get_standardized_score_info(self.total_amounts_liquidation)

        self.t_score = {self.data[i]['a.address']:((self.total_amounts_liquidation[i] - self.mns) / self.sstd * 100 + 500)
