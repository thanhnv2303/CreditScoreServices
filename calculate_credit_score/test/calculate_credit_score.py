...
import numpy as np
from calculate_x2 import CalculateX2
from calculatex51 import CalculateX51
...
class calculate_x134():
    ...
    def get_x4(self, address):
        ...
        while a>0:
            ...
            if (self.listBalanceCLV[a]<self.listBalanceCLV[a-1]) and c>0 and (self.listBorrowCLT[c-1]==self.listBalanceCLT[a-1]):
                c=c-1
                borrow = self.listBorrowCLV[c]
            if (self.listBalanceCLV[a]>self.listBalanceCLV[a-1]) and b>0 and (self.listDepositCLT[b-1]==self.listBalanceCLT[a-1]):
                b=b-1
    ...
    def get_asset(self, address):
        ...
        if (c<0):
            ...
            borrow = self.listBorrowCLV[c]
        while a>0:
            asset+=(self.listBalanceCLV[a]+deposit-borrow)*(self.listBalanceCLT[a-1]-self.listBalanceCLT[a])/(self.k*86400)
            if (self.listBalanceCLV[a]<self.listBalanceCLV[a-1]) and c>0 and (self.listBorrowCLT[c-1]==self.listBalanceCLT[a-1]):
                c=c-1
                borrow = self.listBorrowCLV[c]
            if (self.listBalanceCLV[a]>self.listBalanceCLV[a-1]) and b>0 and (self.listDepositCLT[b-1]==self.listBalanceCLT[a-1]):
                ...
                deposit = self.listDepositCLT[b]
            a+=-1
        asset+=(self.listBalanceCLV[0]+deposit-borrow)*(self.current_timestamp-self.listBalanceCLT[0])/(self.k*86400)
        return asset
    ...
    def get_x31(self, address):
        ...
        for f in range(len(self.borrow_and_balance_history)-1):
            ...
            if (self.borrow_and_balance_history[f+1]==0):
                if (self.borrow_and_balance_history[f]==0):
                    x31+=self.listBorrowCLV[j]*(self.listBalanceCLT[i-1]-present_timestamp)/(86400*self.k*self.listBalanceCLV[i])
                    i+=-1
                else:
                    x31+=self.listBorrowCLV[j]*(self.listBalanceCLT[i]-present_timestamp)/(86400*self.k*self.listBalanceCLV[i])
                    j+=-1

            else:
                if (self.borrow_and_balance_history[f]==1):
                    x31+=self.listBorrowCLV[j]*(self.listBorrowCLT[j-1]-present_timestamp)/(86400*self.k*self.listBalanceCLV[i])
                    j+=-1
                else:
                    x31+=self.listBorrowCLV[j]*(self.listBorrowCLT[j]-present_timestamp)/(86400*self.k*self.listBalanceCLV[i])
                    i+=-1

        last_index=len(self.borrow_and_balance_history)-1
        if (self.borrow_and_balance_history[last_index]==0):
            
            x31+=self.listBorrowCLV[j]/self.listBalanceCLV[i]*(self.current_timestamp- self.listBalanceCLT[i])/(86400*self.k)
        else:
            
            x31+=self.listBorrowCLV[j]/self.listBalanceCLV[i]*(self.current_timestamp- self.listBorrowCLT[j])/(86400*self.k)     
    ...
    def get_x32(self, address):
        ...
        for f in range(len(self.borrow_and_deposit_history)-1):
            ...
            if (self.borrow_and_deposit_history[f+1]==0):
                if (self.borrow_and_deposit_history[f]==0):
                    x32+=self.listBorrowCLV[j]*(self.listDepositCLT[i-1]-present_timestamp)/(86400*self.k*self.listDepositCLV[i])
                    i+=-1
                else:
                    x32+=self.listBorrowCLV[j]*(self.listDepositCLT[i]-present_timestamp)/(86400*self.k*self.listDepositCLV[i])
                    j+=-1
            else:
                if (self.borrow_and_deposit_history[f]==1):
                    x32+=self.listBorrowCLV[j]*(self.listBorrowCLT[j-1]-present_timestamp)/(86400*self.k*self.listDepositCLV[i])
                    j+=-1
                else:
                    x32+=self.listBorrowCLV[j]*(self.listBorrowCLT[j]-present_timestamp)/(86400*self.k*self.listDepositCLV[i])
                    i+=-1
                    
        last_index=len(self.borrow_and_deposit_history)-1
        if (self.borrow_and_deposit_history[last_index]==0):
            
            x32+=self.listBorrowCLV[j]/self.listDepositCLV[i]*(self.current_timestamp- self.listDepositCLT[i])/(86400*self.k)
        else:
            
            x32+=self.listBorrowCLV[j]/self.listDepositCLV[i]*(self.current_timestamp- self.listBorrowCLT[j])/(86400*self.k)     
